...
## What's in it
- **Today** — a day-by-day view with the map pinned on top, a compressed day selector, and the timeline below (on desktop the map sits beside it). Colour-coded emoji pins, the current day/item highlighted live during the trip, and a countdown before it starts. Tapping any item opens a full-screen detail view with a map on the top half.
...
- **Map** — every located stop pinned with the type emoji, one-tap copyable addresses and "open in Google Maps" links, plus a Satellite toggle.
- **Access passcode** — optional lock screen (Settings), shared with Elisa; soft client-side protection on top of the unguessable invite link.
